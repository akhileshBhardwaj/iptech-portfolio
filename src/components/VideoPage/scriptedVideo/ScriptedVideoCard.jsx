...
function ScriptedVideoCard({ video, isActive, onActivate }) {
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Center play/pause icon — no background/shadow now, just
                the icon itself with a drop-shadow for legibility on
                any thumbnail. Visible only while paused; while
                playing it's hidden (opacity-0 + pointer-events-none)
                so it doesn't sit on top of the video. Tapping
                anywhere in this area (handled above) pauses & brings
                it back — state-driven, not hover-driven, so it works
                identically on phones. */}
            <button
              type="button"
              onClick={togglePlay}
              aria-label={
...
export default memo(ScriptedVideoCard);
