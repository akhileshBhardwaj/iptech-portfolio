...
import ProductShootsCard from "./ProductShootsCard";
import productShootsData from "./productShootsData";
...
const ProductShoots = () => {
  return (
    <section
      id="product-shoots-showcase"
      aria-labelledby="product-shoots-showcase-heading"
    ...
    >
      ...
      >
        <motion.h2
          id="product-shoots-showcase-heading"
          variants={headerItem}
          className="font-display lg:mb-15 text-2xl font-bold lg:font-bold leading-[1.1] tracking-tight text-[#12131A] sm:text-[2.25rem]"
        >
          Product <span className="text-[#FF7A45] underline">Shoots</span>
        </motion.h2>
      </motion.div>
      ...
      >
        {productShootsData.map((item) => (
          <ProductShootsCard key={item.id} item={item} />
        ))}
      </motion.div>
